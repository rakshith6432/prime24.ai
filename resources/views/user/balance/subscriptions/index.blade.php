				...
				<div class="tabs-menu ">								
					<ul class="nav">
						@if (config('payment.payment_option') == 'subscription' || config('payment.payment_option') == 'both')
							<li><a href="#plans" class="active" data-toggle="tab"> Plans</a></li>
						@endif
						@if (config('payment.payment_option') == 'prepaid' || config('payment.payment_option') == 'both')
							<li><a href="#prepaid" data-toggle="tab"> Pre Paid</a></li>
						@endif					
					</ul>
				</div>
			</div>
											...
											<div class="col-md-3 col-sm-12">
												<div class="price-card pl-6 pr-6 pt-2 mb-7">
													<div class="card border-0 p-4 pl-6">
														<div class="plan">																										
															<span class="plan-currency-sign">$</span><span class="plan-cost">{{ $subscription->cost }}</span><span class="plan-currency-sign">{{ $subscription->currency }}</span>
						...
						@if (config('payment.payment_option') == 'prepaid' || config('payment.payment_option') == 'both')
							<div class="tab-pane" id="prepaid">
								...
								@if ($prepaids->count())
									...
									<h6 class="font-weight-normal fs-12 text-center mb-6">{{ __('Top up your subscription with more credits or start with') }} <span class="text-info">Pre-Paid</span> {{ __('credits only') }}</h6>
									<div class="row">
												...
												<div class="price-card pl-6 pr-6 pt-2 mb-7">
													<div class="card border-0 p-4 pl-6">
														<div class="plan prepaid-plan">
															<div class="plan-title">{{ $prepaid->plan_name }} <span class="prepaid-currency-sign">{{ $prepaid->currency }}</span><span class="plan-cost">{{ $prepaid->cost }}</span><span class="prepaid-currency-sign">$</span></div>
															<p class="fs-12 text-muted plan-note">{{ __('Total Characters') }} @if ($prepaid->bonus > 0) <span class="ml-2 gift text-success">+{{ number_format($prepaid->bonus) }} {{ __('bonus') }}!</span></p>
